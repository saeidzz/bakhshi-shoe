package com.bakhshi.Main.ui.material.frame;
...
import com.bakhshi.Main.ui.material.frame.transactions.FrameTransactions;
...
public class Frames {
	 ...
	 */
	private void initialize(final List<Frame> frames) {

		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				Frames.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] { " نام تولید کننده ", " تعداد ",
				" سایز ", " کد قالب ", " شماره ", " ردیف " };

		data = new Object[frames.size()][6];

		int i = 0;
		for (Frame frame : frames) {
			data[i] = new Object[] { frame.getProducer(), frame.getNumber(),
					frame.getSize(), frame.getCode(), frame.getId(), i };
			i++;
		}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (frames.get(selectedRow) != null) {
					FrameDao.delete(frames.get(selectedRow).getId());
					frame.dispose();
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (frames.get(selectedRow) != null) {
					new EditFrame(frames.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(frames.get(selectedRow)!=null){
					 new  FrameTransactions(FrameTransactionsDao.read("خروجی", frames.get(selectedRow)),frames.get(selectedRow),"خروجی");
					 frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		button_4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(frames.get(selectedRow)!=null){
					 new  FrameTransactions(FrameTransactionsDao.read("ورودی", frames.get(selectedRow)),frames.get(selectedRow),"ورودی");
				     frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addComponent(button_3, GroupLayout.DEFAULT_SIZE, 181, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_4, GroupLayout.DEFAULT_SIZE, 172, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 117, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 140, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.RELATED)
