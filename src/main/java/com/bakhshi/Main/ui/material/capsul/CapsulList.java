package com.bakhshi.Main.ui.material.capsul;
...
import com.bakhshi.Main.ui.material.capsul.transActions.CapsulTransactions;
...
public class CapsulList {
	...
	public CapsulList(List<Capsul> capsuls) {
		initialize(capsuls);
	}
	 ...
	 */
	private void initialize(final List<Capsul> capsuls) {

		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				CapsulList.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] {  " تعداد ",
				" سایز ", " کد کپسول ", " شماره ", " ردیف " };

		data = new Object[capsuls.size()][5];

		int i = 0;
		for (Capsul capsul : capsuls) {
			data[i] = new Object[] { capsul.getNumber(), capsul.getSize(),
					capsul.getCode(), capsul.getId(), i
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (capsuls.get(selectedRow) != null) {
					CapsulDao.delete(capsuls.get(selectedRow).getId());
					frame.dispose();
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (capsuls.get(selectedRow) != null) {
					new EditCapsul(capsuls.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		JButton button_3 = new JButton("نمایش خروجی ها");
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (capsuls.get(selectedRow) != null) {
					
					new CapsulTransactions(CapsulTransactionDao.read("خروجی",
							capsuls.get(selectedRow)), "خروجی", capsuls
							.get(selectedRow));
					frame.dispose();
		...
		JButton button_4 = new JButton("نمایش ورودی ها");
		button_4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (capsuls.get(selectedRow) != null) {
					frame.dispose();
					new CapsulTransactions(CapsulTransactionDao.read("ورودی",
							capsuls.get(selectedRow)), "ورودی", capsuls
							.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					.addGap(22)
					.addComponent(btnNewButton, GroupLayout.DEFAULT_SIZE, 162, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 157, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_3, GroupLayout.DEFAULT_SIZE, 175, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_4, GroupLayout.DEFAULT_SIZE, 153, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 137, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 180, Short.MAX_VALUE)
					.addContainerGap())
		);
