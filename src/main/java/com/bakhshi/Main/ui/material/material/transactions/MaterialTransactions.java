...
public class MaterialTransactions {
	...
	public MaterialTransactions(List<MatrialTransaction> transactions,
			String inOrOut, com.bakhshi.Main.Model.materials.material.Materials material) {
		initialize(transactions, inOrOut, material);
	}

	/**
	 * Initialize the contents of the frame.
	 */
	private void initialize(final List<MatrialTransaction> transactions,
			final String inOrOut, final com.bakhshi.Main.Model.materials.material.Materials material) {

		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				MaterialTransactions.class
						.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] { "تاریخ", "ورودی/خروجی", "توسط"," وزن کل ", " تعداد ", " نوع مواد ", " شماره ", " ردیف ", };

		data = new Object[transactions.size()][8];

		int i = 0;
		for (MatrialTransaction transaction : transactions) {
			data[i] = new Object[] { transaction.getDate().toString(),
					transaction.getInputOrOutput(), transaction.getBy(),
					transaction.getWeight(), transaction.getNumber(),
					transaction.getMaterial().getType(), transaction.getId(), i };
			i++;
		}
		...
		panel.getTable().getColumnModel().getColumn(5).setCellRenderer(new SimpleTableCellRender());
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (transactions.get(selectedRow) != null) {
					MaterialTransactionDao.delete(transactions.get(selectedRow)
							.getId());
					frame.dispose();

					new MaterialTransactions(MaterialTransactionDao.read(
							inOrOut, material), inOrOut, material);
				}else{
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AddNewMaterialTransaction(material,inOrOut);
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (transactions.get(selectedRow) != null) {
					new EditMaterialTransaction(transactions.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		gl_panel_1.setHorizontalGroup(gl_panel_1.createParallelGroup(
				...
				gl_panel_1
						...
						.addGap(18)
						.addComponent(button, GroupLayout.PREFERRED_SIZE, 261,
								GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.UNRELATED)
						.addComponent(button_1, GroupLayout.PREFERRED_SIZE,
								359, GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.UNRELATED)
						.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 218,
