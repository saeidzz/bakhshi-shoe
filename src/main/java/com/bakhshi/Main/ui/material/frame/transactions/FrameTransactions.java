...
public class FrameTransactions {
	...
	public FrameTransactions(List<FrameTransAction> transActions,Frame fra,String inOout) {
		initialize(transActions,fra,inOout);
	}

	/**
	 * Initialize the contents of the frame.
	 */
	private void initialize(final List<FrameTransAction> transActions,final Frame fra,final String inOout) {

		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				FrameTransactions.class
						.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] { " تاریخ ", " تعداد ",
				"ورودی/خروجی", " کد قالب ", " شماره ", " ردیف " };

		data = new Object[transActions.size()][6];

		int i = 0;
		for (FrameTransAction frame : transActions) {
			data[i] = new Object[] { frame.getDate().toString(),
					frame.getNumber(), frame.getInputOrOutput(),
					frame.getFrame().getCode(), frame.getId(), i };
			i++;
		}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (transActions.get(selectedRow) != null) {
					FrameTransactionsDao.delete(transActions.get(selectedRow).getId());
					frame.dispose();
					new FrameTransactions(FrameTransactionsDao.read(inOout,fra),fra,inOout);
				}else{
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AddNewFrameTransaction(fra,inOout);
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (transActions.get(selectedRow) != null) {
					new EditFrameTransaction(transActions.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 243, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 292, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 166, Short.MAX_VALUE)
					.addContainerGap())
		);
