package com.bakhshi.Main.ui.material.frame.transactions;
...
import com.bakhshi.Main.data_access.FrameTransactionsDao;
...
public class EditFrameTransaction {
	private JFrame frame;
	private JTextField sizeF;
	private JTextField numberF;
	private JTextField frameCode;
	private JTextField producerF;
	private JTextField inOrOut;
	private JTextField yearF;
	private JTextField monthF;
	private JTextField dayF;

	public EditFrameTransaction(FrameTransAction transAction) {
	 ...
	 */
	private void initialize(final FrameTransAction t) {
		frame = new JFrame();
		...
		saveBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String inOrOutv = inOrOut.getText();
				...
				if (inOrOutv.equals("ورودی")) {
					int n = t.getFrame().getNumber();
					n += number;
					t.getFrame().setNumber(n);
				} else if (inOrOutv.equals("خروجی")) {
					int n = t.getFrame().getNumber();
					n -= number;
					t.getFrame().setNumber(n);
				}
				FrameDao.create(t.getFrame());

				FrameTransAction transAction = new FrameTransAction(date, t
						.getFrame(), number, inOrOutv);
				transAction.setId(t.getId());
				FrameTransactionsDao.create(transAction);
				new FrameTransactions(FrameTransactionsDao.read(
						t.getInputOrOutput(), transAction.getFrame()),
						transAction.getFrame(), transAction.getInputOrOutput());
				frame.dispose();
			}
		});
		...
		JLabel label = new JLabel("نام تولید کننده :");
		label.setFont(new Font("Tahoma", Font.BOLD, 15));

		frameCode = new JTextField();
		frameCode.setEditable(false);
		frameCode.setFont(new Font("B Nazanin", Font.BOLD, 18));
		frameCode.setText(t.getFrame().getCode());
		frameCode.setColumns(10);

		JLabel label_1 = new JLabel("کد قالب :");
		label_1.setFont(new Font("Tahoma", Font.BOLD, 15));

		producerF = new JTextField();
		producerF.setEditable(false);
		producerF.setFont(new Font("B Nazanin", Font.BOLD, 18));
		producerF.setColumns(10);
		producerF.setText(t.getFrame().getProducer());

		inOrOut = new JTextField();
		inOrOut.setEditable(false);
		inOrOut.setFont(new Font("B Nazanin", Font.BOLD, 18));
		inOrOut.setText(t.getInputOrOutput());
		inOrOut.setColumns(10);

		JLabel label_2 = new JLabel("ورودی یا خروجی :");
		label_2.setFont(new Font("Tahoma", Font.BOLD, 15));

		yearF = new JTextField();
		yearF.setText(t.getDate().getYear() + "");
		...
		monthF.setText(t.getDate().getMonth() + "");
		monthF.setColumns(10);

		dayF = new JTextField();
		dayF.setText(t.getDate().getYear() + "");
