...
public class Contacts {
	 ...
	 */
	private void initialize(final List<Contact> contacts) {
		
		
		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				Contacts.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] {  
				"شماره تلفن","نام مخاطب", "آدرس","شماره", "ردیف"};

		
		data = new Object[contacts.size()][5];

		int i = 0;
		for (Contact customer : contacts) {
			data[i] = new Object[] {customer.getPhoneNumber(),customer.getName(),customer.getAddress(),customer.getId(),i};
			i++;
		}
			...
			groupLayout.createParallelGroup(Alignment.TRAILING)
				.addGroup(groupLayout.createSequentialGroup()
					.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
						.addComponent(panel_1, GroupLayout.PREFERRED_SIZE, 742, Short.MAX_VALUE)
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(contacts.get(selectedRow)!=null){
				ContactDao.delete(contacts.get(selectedRow).getId());
				frame.dispose();
				new Contacts(ContactDao.read());
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک مخاطب را انتخاب نمایید !!");
				}
				
			}
		});

		JButton button = new JButton("اضافه کردن مخاطب جدید");
		...
		JButton button_1 = new JButton("به روز رسانی مخاطب انتخاب شده");
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (contacts.get(selectedRow)!=null){
					
					new EditContact(contacts.get(selectedRow));
					frame .dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک مخاطب را انتخاب نمایید !!");
				}
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addGap(18)
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 207, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 269, Short.MAX_VALUE)
					.addGap(10)
					.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 100, Short.MAX_VALUE)
					.addContainerGap())
		);
