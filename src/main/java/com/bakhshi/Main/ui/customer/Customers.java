package com.bakhshi.Main.ui.customer;
...
public class Customers {
	...
	private void initialize(final List<Customer> customers) {
		...
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				Customers.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] {  "شماره تلفن", "آدرس" ,"نام مشتری","شماره","ردیف"};

		data = new Object[customers.size()][5];

		int i = 0;
		for (Customer customer : customers) {
			data[i] = new Object[] {  customer.getPhoneNumber(),customer.getAddress(),customer.getName(),customer.getId(), i
					  };
			i++;
		}
			...
			groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup()
					.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
						...
						.addComponent(panel_1, Alignment.LEADING, GroupLayout.DEFAULT_SIZE, 1027, Short.MAX_VALUE))
					.addGap(0))
		);
		...
		JButton btnNewButton = new JButton("حذف مشتری انتخاب شده");
		btnNewButton.setBackground(Color.WHITE);
		btnNewButton.setFont(new Font("Tahoma", Font.BOLD, 15));
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (customers.get(selectedRow) != null) {
					CustomerDao.delete(customers.get(selectedRow).getId());
					frame.dispose();
					new Customers(CustomerDao.read());

				} else {
					JOptionPane.showMessageDialog(frame,
							"لطفا یک مشتری را انتخاب نمایید !!");
				}
			}
		});

		JButton button = new JButton("اضافه کردن مشتری جدید");
		...
		JButton button_1 = new JButton("به روز رسانی مشتری انتخاب شده");
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					.addContainerGap()
					.addComponent(btnNewButton, GroupLayout.DEFAULT_SIZE, 256, Short.MAX_VALUE)
					.addGap(18)
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 254, Short.MAX_VALUE)
					.addGap(18)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 329, Short.MAX_VALUE)
					.addGap(18)
					.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 262, Short.MAX_VALUE)
					.addGap(22))
		);
