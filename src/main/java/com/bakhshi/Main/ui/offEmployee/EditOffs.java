...
import com.bakhshi.Main.ui.AbsentEmployee.EditAbsent;
...
public class EditOffs {
	 ...
	 */
	private void initialize(final Off off) {
		...
		nameF.setColumns(10);
		nameF.setText(off.getEmployee().getName());

		dayF = new JTextField();
		dayF.setFont(new Font("B Nazanin", Font.BOLD, 18));
		dayF.setColumns(10);

		JLabel label_3 = new JLabel("تاریخ مرخصی :");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (day.equals("") || month.equals("") || year.equals("")) {
					...
					Off off1 = new Off(date, off.getEmployee());
					off1.setId(off.getId());
					OffDao.create(off1);
					
					new Offs(OffDao.findByEmployeeIdId(off.getEmployee()), off
							.getEmployee());
					frame.dispose();
		...
		yearF.setColumns(10);
		yearF.setText(off.getOffDay().getYear() + "");
		monthF.setText(off.getOffDay().getMonth() + "");
		dayF.setText(off.getOffDay().getDay() + "");
