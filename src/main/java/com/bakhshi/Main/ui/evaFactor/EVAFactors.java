...
public class EVAFactors {
	...
	public EVAFactors(List<EVASaleFactor> evaSaleFactors) {
		initialize(evaSaleFactors);
	}

	/**
	 * Initialize the contents of the frame.
	 */
	private void initialize(final List<EVASaleFactor> evaSaleFactors) {
		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				EVAFactors.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] {" تاریخ ", " قیمت کل ", " فی ", " تعداد ","سایز", " مدل "," نام ", "شماره فاکتور  "," ردیف " };

		data = new Object[evaSaleFactors.size()][8];
		totalField = new JTextField();
		totalField.setEditable(false);
		totalField.setFont(new Font("B Nazanin", Font.BOLD, 18));
		totalField.setColumns(20);
		int i = 0;
		double totalPrice = 0;
		for (EVASaleFactor factor : evaSaleFactors) {
			data[i] = new Object[] {

			factor.getDate().toString(),
			String.format("%,.0f", factor.getWholePrice()),
			String.format("%,.0f", factor.getUnitPrice()),
			String.format("%.0f", factor.getNumber()),factor.getSize(),
					factor.getProductModel(), factor.getCustomer().getName(),
					factor.getId(), i };
			totalPrice += factor.getWholePrice();
			i++;
		}
		totalField.setText(String.format("%,.0f",totalPrice));
		...
		JLabel lblNewLabel = new JLabel(" : جمع کل ");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (evaSaleFactors.get(selectedRow) != null) {
					EvaFactorDao
							.delete(evaSaleFactors.get(selectedRow).getId());
					frame.dispose();
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AddNewEVAFactor(CustomerDao.read());
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (evaSaleFactors.get(selectedRow) != null) {
					new EditEvaFactor(evaSaleFactors.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new EVaPeriodicReport(CustomerDao.read());
				frame.dispose();
			}
		});
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 208, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 270, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(btnNewButton_1, GroupLayout.PREFERRED_SIZE, 223, GroupLayout.PREFERRED_SIZE)
					.addContainerGap())
		);
