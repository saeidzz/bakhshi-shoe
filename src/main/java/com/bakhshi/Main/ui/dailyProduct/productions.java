...
public class productions {
	...
	public productions(List<Production> list) {
		initialize(list);
	}
	 ...
	 */
	private void initialize(final List<Production> productions) {
		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				productions.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] { "تاریخ", "نام تولید کننده",
				"سایز", "تعداد", "کد محصول", "شماره", "ردیف" };

		data = new Object[productions.size()][7];

		int i = 0;
		for (Production production : productions) {
			data[i] = new Object[] { production.getDate().toString(),
					production.getProductor(), production.getSize(),
					production.getNumber(), production.getCode(),
					production.getId(), i
		...
		groupLayout.setHorizontalGroup(
			groupLayout.createParallelGroup(Alignment.LEADING)
				.addComponent(panel_1, Alignment.TRAILING, GroupLayout.DEFAULT_SIZE, 1006, Short.MAX_VALUE)
				.addComponent(panel, Alignment.TRAILING, GroupLayout.DEFAULT_SIZE, 1006, Short.MAX_VALUE)
		);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (productions.get(selectedRow) != null) {
					ProductionfDao.delete(productions.get(selectedRow).getId());
					frame.dispose();
		...
		JButton button = new JButton("اضافه کردن جدید");
		button.setBackground(Color.WHITE);
		button.setFont(new Font("Tahoma", Font.BOLD, 15));
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AddNewProduction();
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (productions.get(selectedRow) != null) {
					new EditProduction(productions.get(selectedRow));
					frame.dispose();
				} else {
					JOptionPane.showMessageDialog(frame,
							"لطفا یک سطر را انتخاب کنید !!");
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addGap(18)
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 265, Short.MAX_VALUE)
					.addGap(18)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 229, Short.MAX_VALUE)
					.addGap(29)
					.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 231, Short.MAX_VALUE)
					.addGap(25))
		);
