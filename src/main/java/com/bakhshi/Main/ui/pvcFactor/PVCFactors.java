...
public class PVCFactors {
	...
	public PVCFactors(List<PVCSaleFactor> pvcSaleFactors) {
		initialize(pvcSaleFactors);
	}

	/**
	 * Initialize the contents of the frame.
	 */
	private void initialize(final List<PVCSaleFactor> pvcSaleFactor) {

		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				PVCFactors.class.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] { " تاریخ ", " قیمت کل ", " فی "," تعداد ", "سایز", " مدل ", " نام ", "شماره فاکتور  ", " ردیف " };

		data = new Object[pvcSaleFactor.size()][8];

		int i = 0;
		double total = 0;
		for (PVCSaleFactor factor : pvcSaleFactor) {
			data[i] = new Object[] {
					factor.getDate() != null ? factor.getDate().toString() : "",
							String.format("%,.0f", factor.getWholePrice()),
							String.format("%,.0f", factor.getUnitPrice()),
							String.format("%.0f",  factor.getNumber()),
					factor.getSize(), factor.getProductModel(),
					factor.getCustomer().getName(), factor.getId(), i
										...
										.addGroup(
												groupLayout
														...
														.addComponent(
																...
																Short.MAX_VALUE))
										.addContainerGap())
						.addComponent(panel, GroupLayout.DEFAULT_SIZE, 957,
								Short.MAX_VALUE));
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(
				Alignment.TRAILING).addGroup(
				groupLayout
						.createSequentialGroup()
						.addComponent(panel, GroupLayout.DEFAULT_SIZE, 466,
								Short.MAX_VALUE)
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (pvcSaleFactor.get(selectedRow) != null) {
					PvcFactorDao.delete(pvcSaleFactor.get(selectedRow).getId());
					frame.dispose();
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new AddNewPVCFactor(CustomerDao.read());
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (pvcSaleFactor.get(selectedRow) != null) {
					...
					new EditPVCFactor(pvcSaleFactor.get(selectedRow));
					frame.dispose();
				}else{
					JOptionPane.showMessageDialog(frame, "لطفا یک سطر را انتخاب نمایید !!");
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new PvcPeriodicReport(CustomerDao.read());
				frame.dispose();
			}
		});
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button, GroupLayout.PREFERRED_SIZE, 266, GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 239, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addComponent(btnNewButton_1, GroupLayout.DEFAULT_SIZE, 277, Short.MAX_VALUE)
					.addContainerGap())
		);
