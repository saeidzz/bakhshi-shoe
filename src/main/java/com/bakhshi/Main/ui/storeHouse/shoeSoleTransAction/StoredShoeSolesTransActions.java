...
public class StoredShoeSolesTransActions {
	...
	public StoredShoeSolesTransActions(
			List<StoredShoeSoleTransaction> transActions,
			StoredShoeSole shoeSole) {
		initialize(transActions, shoeSole);
	}

	private void initialize(final List<StoredShoeSoleTransaction> transActions,
			final StoredShoeSole shoeSole) {
		frame = new JFrame();
		...
		String[] columnNames = new String[] { "نوع جابه جایی",
				"تعداد جابه جا شده", " تاریخ جابه جایی ", "سایز زیره",
				"رنگ زیره", "کدزیره", " ردیف " };

		data = new Object[transActions.size()][7];

		int i = 0;
		for (StoredShoeSoleTransaction trans : transActions) {
			data[i] = new Object[] { trans.getInOrOut()
					, trans.getNumber(),
					trans.getDate().toString(),
					trans.getStoredShoeSole().getSize(),
					trans.getStoredShoeSole().getColor(),
					trans.getStoredShoeSole().getCode(), i };
			i++;
		}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (transActions.get(selectedRow) != null) {
					StoredShoeSoleTransactionDao.delete(transActions.get(
							selectedRow).getId());
					frame.dispose();
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new AddNewStoredShoeSoleTransaction(shoeSole);
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (transActions.get(selectedRow) != null) {
					...
					new EditStoredShoeSoleTransaction(transActions
							.get(selectedRow));
					frame.dispose();
				}
			}
		...
		gl_panel_1.setHorizontalGroup(gl_panel_1.createParallelGroup(
				...
				gl_panel_1
						...
						.addComponent(btnNewButton, GroupLayout.DEFAULT_SIZE,
								276, Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(button, GroupLayout.DEFAULT_SIZE, 287,
								Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 212,
								Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 136,
