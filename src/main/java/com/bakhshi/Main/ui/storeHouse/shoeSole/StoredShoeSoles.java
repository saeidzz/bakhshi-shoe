package com.bakhshi.Main.ui.storeHouse.shoeSole;
...
import com.bakhshi.Main.ui.storeHouse.shoeSoleTransAction.StoredShoeSolesTransActions;
...
public class StoredShoeSoles {
	...
	public StoredShoeSoles(List<StoredShoeSole> shoeSoles) {
		initialize(shoeSoles);
	}

	private void initialize(final List<StoredShoeSole> shoeSoles) {
		frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(
				AddNewStoredShoeSole.class
						.getResource("/com/bakhshi/Main/ui/icon.jpg")));
		frame.getContentPane().setBackground(Color.WHITE);
		frame.setVisible(true);

		String[] columnNames = new String[] { " موجودی ", "رنگ", "سایز",
				"کد زیره", " شماره  ", " ردیف " };

		data = new Object[shoeSoles.size()][6];

		int i = 0;
		for (StoredShoeSole shoeSole : shoeSoles) {
			data[i] = new Object[] { shoeSole.getNumber(), shoeSole.getColor(),
					shoeSole.getSize(), shoeSole.getCode(), shoeSole.getId(), i };
			i++;
		}
		...
		groupLayout.setHorizontalGroup(groupLayout
				...
				.addComponent(panel_1, GroupLayout.DEFAULT_SIZE, 889,
						Short.MAX_VALUE)
				.addComponent(panel, GroupLayout.DEFAULT_SIZE, 889,
						Short.MAX_VALUE));
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(
				Alignment.TRAILING).addGroup(
				groupLayout
						.createSequentialGroup()
						.addComponent(panel, GroupLayout.DEFAULT_SIZE, 416,
								Short.MAX_VALUE)
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (shoeSoles.get(selectedRow) != null) {
					StoredShoeSoleDao.delete(shoeSoles.get(selectedRow).getId());
					frame.dispose();
					new StoredShoeSoles(StoredShoeSoleDao.read());
				} else {
					JOptionPane.showMessageDialog(frame,
							"لطفا یک سطر را انتخاب نمایید !!");

				}
			}
		});

		JButton button = new JButton("اضافه کردن زیره جدید");
		button.setBackground(Color.WHITE);
		button.setFont(new Font("Tahoma", Font.BOLD, 15));
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AddNewStoredShoeSole();
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (shoeSoles.get(selectedRow) != null) {
					...
					new EditStoredShoeSole(shoeSoles.get(selectedRow));
					frame.dispose();
				} else {
					JOptionPane.showMessageDialog(frame,
							"لطفا یک سطر را انتخاب نمایید !!");
		...
		JButton showTransActionButton = new JButton("نمایش ورود/خروج");
		showTransActionButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (shoeSoles.get(selectedRow) != null) {
                  new StoredShoeSolesTransActions(StoredShoeSoleTransactionDao.findByShoeSole(shoeSoles.get(selectedRow)),shoeSoles.get(selectedRow));
					frame.dispose();
				} else {
					JOptionPane.showMessageDialog(frame,
							"لطفا یک سطر را انتخاب نمایید !!");
		...
		showTransActionButton.setFont(new Font("Tahoma", Font.BOLD, 15));
		showTransActionButton.setBackground(Color.WHITE);

		JButton button_2 = new JButton("چاپ");
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			  new PrintShoeSoles(StoredShoeSoleDao.read());
			  frame.dispose();
			}
		});
		...
		gl_panel_1.setHorizontalGroup(gl_panel_1.createParallelGroup(
				...
				gl_panel_1
						...
						.addComponent(button, GroupLayout.DEFAULT_SIZE, 223,
								Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.UNRELATED)
						.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 163,
								Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.UNRELATED)
						.addComponent(showTransActionButton,
