...
public class Transactions {
	...
	public Transactions(List<PropertyTransaction> propertyTransactions,
			Property property) {
		initialize(propertyTransactions, property);
	}

	private void initialize(
			final List<PropertyTransaction> propertyTransactions,
		...
		String[] columnNames = new String[] { "نوع جابه جایی",
				"تعداد جابه جا شده", "توسط", " تاریخ جابه جایی ", "نام کالا ",
				" شماره  ", " ردیف " };

		data = new Object[propertyTransactions.size()][7];

		int i = 0;
		for (PropertyTransaction trans : propertyTransactions) {
			data[i] = new Object[] {
			trans.getInOrOut(), trans.getNumber(), trans.getTransactor(),
					trans.getDate().toString(), trans.getProperty().getTitle(),
					trans.getId(), i };
			i++;
		}
		...
		JButton btnNewButton = new JButton("حذف انتخاب شده");
		btnNewButton.setBackground(Color.WHITE);
		btnNewButton.setFont(new Font("Tahoma", Font.BOLD, 15));
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (propertyTransactions.get(selectedRow) != null) {
					PropertyTransactionDao.delete(propertyTransactions.get(
							selectedRow).getId());
					frame.dispose();
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new AddNewTransAction(property);
				frame.dispose();
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (propertyTransactions.get(selectedRow) != null) {
					...
					new EditTransAction(propertyTransactions.get(selectedRow));
					frame.dispose();
				}
			}
		});
		...
		gl_panel_1.setHorizontalGroup(
			gl_panel_1.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_1.createSequentialGroup()
					...
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(button, GroupLayout.DEFAULT_SIZE, 287, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(button_1, GroupLayout.DEFAULT_SIZE, 212, Short.MAX_VALUE)
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(button_2, GroupLayout.DEFAULT_SIZE, 136, Short.MAX_VALUE)
					.addContainerGap())
		);
